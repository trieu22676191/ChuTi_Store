...
import '../Css/NewProductsPage.css'; // Import CSS for styling
...
const NewProductsPage = () => {
  ...
  const [showCount, setShowCount] = useState(null); // Hiển thị tất cả mặc định
  ...
  const sortProducts = (option) => {
    ...
    if (option === 'newest') {
      sortedProducts.sort((a, b) => b.id - a.id); // Sort by newest
    } else if (option === 'bestselling') {
      sortedProducts.sort((a, b) => b.soldCount - a.soldCount); // Sort by best-selling
  ...
  // Lọc sản phẩm theo khoảng giá, brand, category
  ...
  // Tính toán danh sách brand và số lượng sản phẩm theo brand
  const brandMap = {};
  products.forEach((p) => {
    if (p.brand) {
  ...
  // Tính toán danh sách category và số lượng sản phẩm theo category
  const categoryMap = {};
  products.forEach((p) => {
    if (p.category) {
  ...
  // Hàm nhận giá trị lọc giá từ SidebarFilter
  const handlePriceFilter = (min, max) => {
    setFilterPrice({ min, max });
  };

  // Hàm nhận giá trị lọc brand từ SidebarFilter
  const handleBrandFilter = (brandName) => {
    setFilterBrand(brandName);
  };

  // Hàm nhận giá trị lọc category từ SidebarFilter
